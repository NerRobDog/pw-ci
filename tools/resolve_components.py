...
class Resolver(object):
    ...
    def load(self, descriptor, name):
        ...
        for key in ('globalCompilerKeys', 'compilerKeys', 'localCompilerKeys'):
            for k in l.get(key, []) or []:
                comp.defines.append(k)
        for key in ('globalDefines', 'defines', 'localDefines'):
...
def main():
    ...
    for comp in ordered:
        ...
        if comp.pch and own:
            pch_groups.append((comp.pch, own))
    ...
    with open(args.out, 'w') as fh:
        ...
        fh.write('# %d components, %d sources, %d forced-include groups\n\n'
                 % (len(r.by_descriptor), len(srcs), len(pch_groups)))
        ...
        fh.write('macro( %s_apply_pch )\n' % p)
        for pch, files in pch_groups:
    ...
    sys.stderr.write('resolved %d components, %d sources, %d include dirs, %d missing\n'
                     % (len(r.by_descriptor), len(srcs), len(incs), len(r.missing)))
    return 0
